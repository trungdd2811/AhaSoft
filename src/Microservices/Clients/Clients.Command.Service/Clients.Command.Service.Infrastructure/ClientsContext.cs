...
namespace Clients.Command.Service.Infrastructure
{
    ...
    //implementing this class to fix the error when using migration tool
    public class ClientsContextDesignFactory : IDesignTimeDbContextFactory<ClientsContext>
    {
        public ClientsContext CreateDbContext(string[] args)
        {
            var optionsBuilder = new DbContextOptionsBuilder<ClientsContext>()
                .UseSqlServer("Data Source=(localdb)\\MSSQLLocalDb;Initial Catalog=aha.clients;Integrated Security=True");

            return new ClientsContext(optionsBuilder.Options, new NoMediator());
        }
